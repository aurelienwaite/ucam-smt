...
/**
 * Hadoop job to compute add-one smoothed source-to-target probability. 
 * @author Juan Pino
 * @date 28 May 2014
 */
public class Source2TargetAddOneSmoothedJob extends Configured implements Tool {
	...
	private static class KeepProvenanceCountsOnlyAddOneMapper
			extends
		...
		protected void map(RuleWritable key, RuleInfoWritable value,
			...
			ProvenanceCountMap counts = value.getProvenanceCountMap();
			for (ByteWritable prov : counts.keySet()) {
				counts.put(prov, new IntWritable(counts.get(prov).get() + 1));
			}
	...
	public static Job getJob(Configuration conf) throws IOException {
		...
		conf.set(MarginalReducer.FEATURE_NAME, "source2target_addonesmoothed_probability");
		Job job = new Job(conf);
		job.setJarByClass(Source2TargetAddOneSmoothedJob.class);
		job.setJobName("Source2TagetAddOneSmoothed");
		job.setSortComparatorClass(Source2TargetComparator.class);
		job.setPartitionerClass(Source2TargetPartitioner.class);
		job.setMapperClass(KeepProvenanceCountsOnlyAddOneMapper.class);
	...
	public static class Source2TargetAddOneSmoothedJobParameters {
		@Parameter(names = { "--input", "-i" }, description = "Input rules on HDFS", required = true)
		public String input;

		@Parameter(names = { "--output", "-o" }, description = "Output add-one smoothed source-to-target probabilities on HDFS", required = true)
	...
	public int run(String[] args) throws IllegalArgumentException,
		...
		Source2TargetAddOneSmoothedJobParameters params =
				new Source2TargetAddOneSmoothedJobParameters();
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Source2TargetAddOneSmoothedJob(), args);
		System.exit(res);
	}
}
